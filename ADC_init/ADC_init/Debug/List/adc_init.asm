...
; 0000 0012 void show(unsigned int x) {

	.CSEG
_show:
; .FSTART _show
; 0000 0013     unsigned int temp;
; 0000 0014     unsigned char arr[4], i;
; 0000 0015     temp = x;
	ST   -Y,R27
	ST   -Y,R26
	SBIW R28,4
...
;	i -> R19
	__GETWRS 16,17,8
; 0000 0016     for (i = 0; i < 4; i++) // tach so tu duoi lên tren
	LDI  R19,LOW(0)
_0x6:
	CPI  R19,4
	BRSH _0x7
; 0000 0017     {
; 0000 0018         arr[3 - i] = temp % 10;
	MOV  R30,R19
	LDI  R31,0
	LDI  R26,LOW(3)
	LDI  R27,HIGH(3)
...
; 0000 0026    DK3 = 0;
	CBI  0x15,6
; 0000 0027    PORTB = number1[arr[2]];
	LDD  R30,Y+6
	LDI  R31,0
	SUBI R30,LOW(-_number1)
	SBCI R31,HIGH(-_number1)
	LD   R30,Z
	OUT  0x18,R30
...
; .FSTART _adc_isr
	...
	IN   R30,SREG
	ST   -Y,R30
; 0000 003A static unsigned char input_index=0;
; 0000 003B // Read the 8 most significant bits
; 0000 003C // of the AD conversion result
; 0000 003D adc_data[input_index]=ADCH;
	...
	IN   R30,0x5
	ST   X,R30
; 0000 003E // Select next ADC input
; 0000 003F if (++input_index > (LAST_ADC_INPUT-FIRST_ADC_INPUT))
	LDS  R26,_input_index_S0000001000
	SUBI R26,-LOW(1)
	STS  _input_index_S0000001000,R26
	CPI  R26,LOW(0x1)
	BRLO _0x18
; 0000 0040    input_index=0;
	LDI  R30,LOW(0)
	STS  _input_index_S0000001000,R30
; 0000 0041 ADMUX=(FIRST_ADC_INPUT | ADC_VREF_TYPE)+input_index;
_0x18:
...
; 0000 0055 DDRB=(1<<DDB7) | (1<<DDB6) | (1<<DDB5) | (1<<DDB4) | (1<<DDB3) | (1<<DDB2) | (1<<DDB1) | (1<<DDB0);
	LDI  R30,LOW(255)
	OUT  0x17,R30
; 0000 0056 // State: Bit7=1 Bit6=1 Bit5=1 Bit4=1 Bit3=1 Bit2=1 Bit1=1 Bit0=1
...
; 0000 005B DDRC=(1<<DDC7) | (1<<DDC6) | (1<<DDC5) | (1<<DDC4) | (1<<DDC3) | (1<<DDC2) | (1<<DDC1) | (1<<DDC0);
	OUT  0x14,R30
; 0000 005C // State: Bit7=1 Bit6=1 Bit5=1 Bit4=1 Bit3=1 Bit2=1 Bit1=1 Bit0=1
; 0000 005D PORTC=(1<<PORTC7) | (1<<PORTC6) | (1<<PORTC5) | (1<<PORTC4) | (1<<PORTC3) | (1<<PORTC2) | (1<<PORTC1) | (1<<PORTC0);
	OUT  0x15,R30
; 0000 005E 
...
; 0000 00A8 // ADC Auto Trigger Source: Free Running
...
; 0000 00AB ADMUX=FIRST_ADC_INPUT | ADC_VREF_TYPE;
	LDI  R30,LOW(32)
	OUT  0x7,R30
; 0000 00AC ADCSRA=(1<<ADEN) | (1<<ADSC) | (1<<ADATE) | (0<<ADIF) | (1<<ADIE) | (0<<ADPS2) | (1<<ADPS1) | (1<<ADPS0);
	LDI  R30,LOW(235)
	OUT  0x6,R30
...
_0x19:
; 0000 00BB       {
; 0000 00BC         dienap = (50 * adc_data[0]) / 255;
	LDS  R30,_adc_data
	LDI  R26,LOW(50)
	MUL  R30,R26
	MOVW R30,R0
